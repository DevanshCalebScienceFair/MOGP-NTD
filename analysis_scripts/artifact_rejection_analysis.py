...
for s in range(10):
    ...
    b = summarize(f"{B}/artifact_rejection_arm/reject_seed{s}")
    if a is None or b is None: continue
    rows.append(dict(seed=s, **{f"base_{k}": v for k, v in a.items()},
                     **{f"rej_{k}": v for k, v in b.items()}))
if not rows:
    print("No paired seeds yet. Run: ./run_artifact_rejection_arm.sh"); sys.exit(0)
...
df.to_csv(f"{B}/artifact_rejection_arm/scored.csv", index=False)
print(f"\nWrote artifact_rejection_arm/scored.csv")
